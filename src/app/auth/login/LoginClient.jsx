...
export default function LoginClient() {
  ...
  // Where to go after login — support common names
  ...
  // Map NextAuth-style error codes from ?error=
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await signIn('credentials', {
        ...
        redirect: false,                // we’ll navigate ourselves
        callbackUrl,                    // still pass it along for consistency
      });

      if (res?.ok) {
        // Avoid back button returning to login
    ...
    <div className="min-h-screen grid grid-cols-1 md:grid-cols-2">
      ...
      <div className="flex flex-col justify-center items-center p-8 bg-white relative">
        <div className="absolute top-6 right-6">
          <Image src="/logo.png" alt="SituationRoom Logo" width={180} height={40} priority />
        </div>

        <form onSubmit={handleSubmit} className="w-full max-w-sm mt-12 md:mt-0" noValidate>
          ...
          {/* Global form error (aria-live for a11y) */}
          {errors.form && (
          ...
          {touched.password && errors.password && (
            <p className="text-red-600 text-xs" role="alert">{errors.password}</p>
          )}
