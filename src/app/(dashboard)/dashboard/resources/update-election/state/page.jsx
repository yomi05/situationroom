...
export default function ManageGovernorshipPage() {
  ...
  async function load() {
    ...
    try {
      const res = await fetch('/api/past-elections/governorship', { cache: 'no-store' });
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.candidate, r.party, r.state, r.year]
        .filter(Boolean)
  ...
  async function save(e) {
    ...
    try {
      ...
      const url = form._id
        ? `/api/past-elections/governorship/${form._id}`
        : '/api/past-elections/governorship';
  ...
  async function del(id) {
    ...
    setMsg(null); setErr(null);
    try {
      const res = await fetch(`/api/past-elections/governorship/${id}`, { method: 'DELETE' });
      const j = await res.json().catch(()=>({}));
  ...
  async function onCsvPicked(file) {
    ...
    try {
      ...
      const res = await fetch('/api/past-elections/governorship/mass-upload', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Manage — Governorship</h1>
        <div className="flex gap-2">
          <input
            className="rounded-xl border px-3 py-2"
            placeholder="Search candidate, party, state, year…"
            value={search}
        ...
        <div className="fixed inset-0 z-50">
          ...
          <form onSubmit={save} className="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              <input className="rounded-xl border px-3 py-2" placeholder="Deputy" value={form.deputy} onChange={e=>setForm(f=>({...f,deputy:e.target.value}))} />
