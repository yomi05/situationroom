...
export default function ManagePresidentialElectionsPage() {
  const { data: session } = useSession();
  const role = session?.user?.role || 'Guest';

  const canView = hasPerm(role, PERMS.UPDATE_PAST_ELECTIONS); // managers only
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.candidate, r.party, r.position, r.year]
        .filter(Boolean)
  ...
  async function save(e) {
    ...
    try {
      const payload = {
        ...
        votes: form.votes, // may include commas; display handles numeric
        percentage: Number(form.percentage),
      };
      const method = form._id ? 'PUT' : 'POST';
      const url = form._id
        ? `/api/past-elections/presidential/${form._id}`
        : '/api/past-elections/presidential';
  ...
  async function del(id) {
    ...
    setMsg(null); setErr(null);
    try {
      const res = await fetch(`/api/past-elections/presidential/${id}`, { method: 'DELETE' });
      const j = await res.json().catch(()=>({}));
  ...
  // ✅ FIXED: function name and the call site
  async function onCsvPicked(file) {
    if (!file) return;
    try {
      const { parse } = await import('papaparse'); // dynamic to avoid SSR bundling
      ...
      const docs = data
        .map(r => ({
          ...
          party: r.party,
          candidate: r.candidate,
          votes: r.votes, // may be "1,234,567"
          percentage: Number(r.percentage),
        }))
        .filter(d => d.year && d.party && d.candidate && Number.isFinite(d.percentage));

      if (!docs.length) throw new Error('CSV has no valid rows');

      const res = await fetch('/api/past-elections/presidential/mass-upload', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Manage — Presidential</h1>
        <div className="flex gap-2">
          <input
            className="rounded-xl border px-3 py-2"
            placeholder="Search candidate, party, year…"
            value={search}
          ...
          <label className="rounded-xl border px-3 py-2 hover:bg-gray-50 cursor-pointer">
            ...
            <input
              ...
              accept=".csv"
              className="hidden"
              onChange={e => onCsvPicked(e.target.files?.[0])}  // ✅ FIXED call
            />
          </label>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50">
          ...
          <form onSubmit={save} className="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-1 gap-3">
              <input className="rounded-xl border px-3 py-2" placeholder="Position" value={form.position} onChange={e=>setForm(f=>({...f,position:e.target.value}))} />
              ...
              <input className="rounded-xl border px-3 py-2" placeholder="Votes (e.g. 8,794,726)" value={form.votes} onChange={e=>setForm(f=>({...f,votes:e.target.value}))} />
              <input className="rounded-xl border px-3 py-2" placeholder="Percentage (e.g. 36.61)" type="number" step="0.01" value={form.percentage} onChange={e=>setForm(f=>({...f,percentage:e.target.value}))} required />
            </div>
