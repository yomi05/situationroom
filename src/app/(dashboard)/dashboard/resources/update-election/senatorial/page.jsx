...
export default function ManageSenatorialPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.candidate, r.party, r.state, r.district, r.year]
        .filter(Boolean)
  ...
  async function save(e) {
    ...
    try {
      ...
      const url = form._id
        ? `/api/past-elections/senatorial/${form._id}`
        : '/api/past-elections/senatorial';
  ...
  async function del(id) {
    ...
    setMsg(null); setErr(null);
    try {
      const res = await fetch(`/api/past-elections/senatorial/${id}`, { method: 'DELETE' });
      const j = await res.json().catch(()=>({}));
  ...
  async function onCsvPicked(file) {
    ...
    try {
      ...
      const res = await fetch('/api/past-elections/senatorial/mass-upload', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Manage — Senatorial</h1>
        <div className="flex gap-2">
          <input
            className="rounded-xl border px-3 py-2"
            placeholder="Search candidate, party, state, district, year…"
            value={search}
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={r._id} className="border-t hover:bg-gray-50">
                ...
                <td className="p-3">{String(r.votes ?? '').replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}</td>
        ...
        <div className="fixed inset-0 z-50">
          ...
          <form onSubmit={save} className="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              <input className="rounded-xl border px-3 py-2" placeholder="District" value={form.district} onChange={e=>setForm(f=>({...f,district:e.target.value}))} required />
