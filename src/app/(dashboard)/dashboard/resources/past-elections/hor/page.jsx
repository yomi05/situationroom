...
export default function HorViewPage() {
  ...
  const constituenciesForStateYear = useMemo(() => {
    const pool = rows.filter(r =>
      (!yearFilter || String(r.year) === String(yearFilter)) &&
      (!stateFilter || r.state === stateFilter)
    );
    return Array.from(new Set(pool.map(r => String(r.constituency||'').trim()))).filter(Boolean).sort();
  ...
  const filtered = useMemo(() => {
    return rows.filter(r =>
      (!yearFilter || String(r.year) === String(yearFilter)) &&
      (!stateFilter || r.state === stateFilter) &&
      (!constFilter || r.constituency === constFilter)
    );
  }, [rows, yearFilter, stateFilter, constFilter]);

  const labels = useMemo(
    () => filtered.map(r => `${r.candidate} (${r.state}, ${r.constituency})`),
    [filtered]
  );
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Past Elections — House of Representatives</h1>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        <select className="rounded-xl border px-3 py-2" value={yearFilter} onChange={e=>{setYearFilter(e.target.value); setConstFilter('');}}>
          <option value="">All Years</option>
          {years.map(y => <option key={`year-${y}`} value={String(y)}>{y}</option>)}
        </select>

        <select className="rounded-xl border px-3 py-2" value={stateFilter} onChange={e=>{setStateFilter(e.target.value); setConstFilter('');}}>
          <option value="">All States</option>
          {states.map(s => <option key={`state-${s}`} value={s}>{s}</option>)}
        </select>

        <select className="rounded-xl border px-3 py-2" value={constFilter} onChange={e=>setConstFilter(e.target.value)}>
          <option value="">All Constituencies</option>
          {constituenciesForStateYear.map(c => <option key={`const-${c}`} value={c}>{c}</option>)}
        </select>

        <button className="rounded-xl border px-3 py-2" onClick={()=>{ setYearFilter(''); setStateFilter(''); setConstFilter(''); }}>
          Clear Filters
        </button>
      </div>
