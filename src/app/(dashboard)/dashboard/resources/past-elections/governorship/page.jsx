...
export default function GovernorshipViewPage() {
  ...
  const labels = useMemo(() => filtered.map(r => `${r.candidate} (${r.state})`), [filtered]);
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Past Elections — Governorship</h1>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <select className="rounded-xl border px-3 py-2" value={yearFilter} onChange={e=>setYearFilter(e.target.value)}>
        ...
        <button className="rounded-xl border px-3 py-2" onClick={()=>{ setYearFilter(''); setStateFilter(''); }}>
          Clear Filters
        </button>
      </div>
